<template>
  <transition name="w-alert-fade">
    <div class="w-alert" :class="[ typeClass, {'is-center': center} ]" v-show="visible">
      <i class="w-alert__icon" :class="[ iconClass, isBigIcon ]" v-if="showIcon"></i>
      <div class="w-alert__content">
        <span class="w-alert__title" :class="isBoldTitle" style>{{this.title}}</span>
        <p class="w-alert__description" v-if="description">{{description}}</p>
        <i
          v-if="closable"
